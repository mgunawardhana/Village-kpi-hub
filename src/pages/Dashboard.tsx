...
import { Leaf, ArrowRight, Loader2, Sparkles } from "lucide-react";
...
const Auth = () => {
    ...
    // Custom animated border wrapper component
    const AnimatedInputWrapper = ({ children }: { children: React.ReactNode }) => (
        <div className="group relative">
            {/* The animated gradient border */}
            <div className="absolute -inset-[2px] rounded-xl bg-gradient-to-r from-amber-500 via-[#1C1917] to-amber-500 opacity-0 group-hover:opacity-70 group-focus-within:opacity-100 transition-opacity duration-500 animate-border-flow bg-[length:200%_200%] blur-[1px] -z-10" />
            {children}
        </div>
    );
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#E6E4DF] p-4 relative overflow-hidden">
            {/* Inject custom keyframes for the flowing border */}
            <style>{`
      ...
      `}</style>
            ...
            <Card className="w-full max-w-md bg-[#FBFBF9] border border-stone-200 shadow-2xl rounded-[32px] relative z-10 overflow-hidden">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <AnimatedInputWrapper>
                                        <Input
                                            ...
                                            required
                                            className="relative h-12 bg-stone-50 border-stone-200 focus:border-transparent focus:ring-0 rounded-xl placeholder:text-stone-400 text-stone-800 z-10 transition-all"
                                        />
                                    </AnimatedInputWrapper>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <AnimatedInputWrapper>
                                        <Select value={role} onValueChange={(value: "admin" | "quality_circle_leader") => setRole(value)}>
                                            <SelectTrigger className="relative h-12 bg-stone-50 border-stone-200 focus:ring-0 focus:border-transparent rounded-xl text-stone-800 z-10">
